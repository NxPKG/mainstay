...
const registrar = new mainstay.web3.Account();
const rewardQ = new mainstay.web3.Account();
...
async function setupStakePool(mint, god) {
  ...
  const seed = mainstay.utils.sha256
    .hash(`${registrar.publicKey.toString()}:Member`)
    .slice(0, 32);
  member = await mainstay.web3.PublicKey.createWithSeed(
  ...
  const tx = registry.transaction.createMember(nonce2, {
    ...
    instructions: [
      mainstay.web3.SystemProgram.createAccountWithSeed({
