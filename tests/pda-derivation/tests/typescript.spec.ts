...
const encode = mainstay.utils.bytes.utf8.encode;

describe("typescript", () => {
  // Configure the client to use the local cluster.
  mainstay.setProvider(mainstay.MainstayProvider.env());

  const program = mainstay.workspace.PdaDerivation as Program<PdaDerivation>;
  ...
  it("Inits the derived accounts", async () => {
    ...
    const expectedPDAKey = PublicKey.findProgramAddressSync(
      [
        ...
        new mainstay.BN(MY_SEED_U32).toArrayLike(Buffer, "le", 4),
        new mainstay.BN(MY_SEED_U64).toArrayLike(Buffer, "le", 8),
        new mainstay.BN(data).toArrayLike(Buffer, "le", 8),
        dataKey.publicKey.toBuffer(),
        new mainstay.BN(anotherData).toArrayLike(Buffer, "le", 8),
      ],
      program.programId
    )[0];
  ...
  it("Can resolve associated token accounts", async () => {
    const mintKp = mainstay.web3.Keypair.generate();
    await program.methods
