...
describe("CPI return", () => {
  ...
  const callerProgram = mainstay.workspace.Caller as Program<Caller>;
  const calleeProgram = mainstay.workspace.Callee as Program<Callee>;
  ...
  const cpiReturn = mainstay.web3.Keypair.generate();
  ...
  it("can return a u64 via view", async () => {
    // @ts-expect-error
    assert(new mainstay.BN(99).eq(await callerProgram.views.returnU64()));
    // Via methods API
    assert(
      new mainstay.BN(99).eq(await callerProgram.methods.returnU64().view())
    );
  });
  ...
  it("can return a struct via view", async () => {
    ...
    assert(struct.a.eq(new mainstay.BN(1)));
    assert(struct.b.eq(new mainstay.BN(2)));
    // Via methods API
    const struct2 = await callerProgram.methods.returnStruct().view();
    assert(struct2.a.eq(new mainstay.BN(1)));
    assert(struct2.b.eq(new mainstay.BN(2)));
  });

  it("can return a vec via view", async () => {
    // @ts-expect-error
    const vec = await callerProgram.views.returnVec();
    assert(vec[0].eq(new mainstay.BN(1)));
    assert(vec[1].eq(new mainstay.BN(2)));
    assert(vec[2].eq(new mainstay.BN(3)));
    // Via methods API
    const vec2 = await callerProgram.methods.returnVec().view();
    assert(vec2[0].eq(new mainstay.BN(1)));
    assert(vec2[1].eq(new mainstay.BN(2)));
    assert(vec2[2].eq(new mainstay.BN(3)));
  });

  it("can return a u64 from an account via view", async () => {
    const value = new mainstay.BN(10);
    assert(
      value.eq(
        await calleeProgram.methods
