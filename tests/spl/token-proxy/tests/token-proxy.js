...
describe("program", () => {
  ...
  const TEST_PROGRAM_IDS = [
    SPL_TOKEN_PROGRAM_ID,
    new mainstay.web3.PublicKey("TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb"),
  ];
  ...
  const program = mainstay.workspace.TokenProxy;
  ...
  TOKEN_PROGRAMS.forEach((tokenProgram) => {
    ...
    describe(name, () => {
      ...
      it("Creates a token account", async () => {
        ...
        const [tokenAccount] = mainstay.web3.PublicKey.findProgramAddressSync(
          [
            authority.toBytes(),
            newMint.toBytes(),
        ...
        assert.isTrue(account.amount.eq(new mainstay.BN(0)));
      });

      it("Creates an associated token account", async () => {
        const newMint = await createMint(tokenProgram);
        const authority = provider.wallet.publicKey;
        const associatedTokenProgram = new mainstay.web3.PublicKey(
      ...
      it("Creates a mint", async () => {
        const authority = provider.wallet.publicKey;
        const [newMint] = mainstay.web3.PublicKey.findProgramAddressSync(
          [
            authority.toBytes(),
            Buffer.from(name),
      ...
      it("Mints a token", async () => {
        await program.rpc.proxyMintTo(new mainstay.BN(1000), {
          accounts: {
        ...
        assert.isTrue(fromAccount.amount.eq(new mainstay.BN(1000)));
      });
      ...
      it("Transfers a token", async () => {
        ...
        const transferAmount = new mainstay.BN(400);
      ...
      it("Transfers a token with optional accounts", async () => {
        ...
        const transferAmount = new mainstay.BN(10);
      ...
      it("Does not transfer a token without optional accounts", async () => {
        ...
        const optionalTransferIx = await program.methods
          .proxyOptionalTransfer(new mainstay.BN(10))
          .accounts({
            authority: provider.wallet.publicKey,
            to,
            from,
          ...
          .instruction();
        const tx = new mainstay.web3.Transaction().add(optionalTransferIx);
      ...
      it("Burns a token", async () => {
        ...
        const burnAmount = new mainstay.BN(300);
      ...
      it("Set new mint authority", async () => {
        const newMintAuthority = mainstay.web3.Keypair.generate();
...
async function createMint(tokenProgram) {
  ...
  const tx = new mainstay.web3.Transaction();
