...
describe("transfer hook", () => {
  ...
  const program = mainstay.workspace.TransferHook as Program<TransferHook>;
  ...
  it("can create an `InitializeExtraAccountMetaList` instruction with the proper discriminator", async () => {
    ...
    const { name, data } = new mainstay.BorshInstructionCoder(program.idl).decode(
      ix.data,
      "hex"
    );
  ...
  it("can create an `Execute` instruction with the proper discriminator", async () => {
    const ix = await program.methods
      .execute(new mainstay.BN(transferAmount))
      .accounts({
    ...
    assert.isTrue(mainstay.BN.isBN(data.amount));
    assert.isTrue(data.amount.eq(new mainstay.BN(transferAmount)));
  });
