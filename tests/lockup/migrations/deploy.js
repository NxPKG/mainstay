...
module.exports = async function (provider) {
  ...
  const defaultEntry = { programId: new mainstay.web3.PublicKey.default() };
  ...
  for (let k = 0; k < cfgKeys.length; k += 1) {
    ...
    const registrar = await registrarInit(
      ...
      r.stakeRate,
      r.rewardQLen,
      new mainstay.web3.PublicKey(r.mint)
    );
...
async function genesis(provider) {
  if (
    ...
    const [token1Mint, _god1] = await serumCmn.createMintAndVault(
      provider,
      new mainstay.BN(10000000000000),
      undefined,
      6
    );
    const [token2Mint, _god2] = await serumCmn.createMintAndVault(
      provider,
      new mainstay.BN(10000000000),
      undefined,
      0
    );
    return {
...
async function registrarInit(
  ...
  const withdrawalTimelock = new mainstay.BN(_withdrawalTimelock);
  const stakeRate = new mainstay.BN(_stakeRate);
