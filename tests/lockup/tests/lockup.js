...
describe("Lockup and Registry", () => {
  ...
  const lockup = mainstay.workspace.Lockup;
  const registry = mainstay.workspace.Registry;
  ...
  it("Is initialized!", async () => {
    ...
    lockupAccount.whitelist.forEach((e) => {
      assert.isTrue(e.programId.equals(mainstay.web3.PublicKey.default));
    });
  });

  it("Deletes the default whitelisted addresses", async () => {
    const defaultEntry = { programId: mainstay.web3.PublicKey.default };
  ...
  it("Sets a new authority", async () => {
    const newAuthority = mainstay.web3.Keypair.generate();
  ...
  it("Adds to the whitelist", async () => {
    const generateEntry = async () => {
      let programId = mainstay.web3.Keypair.generate().publicKey;
      return {
        programId,
      };
    };
  ...
  const vesting = mainstay.web3.Keypair.generate();
  let vestingAccount = null;
  let vestingSigner = null;

  it("Creates a vesting account", async () => {
    const startTs = new mainstay.BN(Date.now() / 1000);
    const endTs = new mainstay.BN(startTs.toNumber() + 5);
    const periodCount = new mainstay.BN(2);
    const beneficiary = provider.wallet.publicKey;
    const depositAmount = new mainstay.BN(100);

    const vault = mainstay.web3.Keypair.generate();
    let [_vestingSigner, nonce] =
      await mainstay.web3.PublicKey.findProgramAddress(
    ...
    assert.isTrue(vestingAccount.whitelistOwned.eq(new mainstay.BN(0)));
    assert.strictEqual(vestingAccount.nonce, nonce);
    assert.isTrue(vestingAccount.createdTs.gt(new mainstay.BN(0)));
  ...
  it("Fails to withdraw from a vesting account before vesting", async () => {
    await nativeAssert.rejects(
      async () => {
        await lockup.rpc.withdraw(new mainstay.BN(100), {
          accounts: {
            vesting: vesting.publicKey,
  ...
  it("Withdraws from the vesting account", async () => {
    ...
    vestingAccount = await lockup.account.vesting.fetch(vesting.publicKey);
    assert.isTrue(vestingAccount.outstanding.eq(new mainstay.BN(0)));
    ...
    assert.isTrue(tokenAccount.amount.eq(new mainstay.BN(100)));
  });

  const registrar = mainstay.web3.Keypair.generate();
  const rewardQ = mainstay.web3.Keypair.generate();
  const withdrawalTimelock = new mainstay.BN(4);
  const stakeRate = new mainstay.BN(2);
  ...
  const member = mainstay.web3.Keypair.generate();
  ...
  it("Creates a member", async () => {
    ...
    assert.isTrue(memberAccount.metadata.equals(mainstay.web3.PublicKey.default));
    assert.strictEqual(
    ...
    assert.isTrue(memberAccount.lastStakeTs.eq(new mainstay.BN(0)));
  });

  it("Deposits (unlocked) to a member", async () => {
    const depositAmount = new mainstay.BN(120);
  ...
  it("Stakes to a member (unlocked)", async () => {
    const stakeAmount = new mainstay.BN(10);
    ...
    assert.isTrue(vault.amount.eq(new mainstay.BN(100)));
    assert.isTrue(vaultStake.amount.eq(new mainstay.BN(20)));
    assert.isTrue(spt.amount.eq(new mainstay.BN(10)));
  });

  const unlockedVendor = mainstay.web3.Keypair.generate();
  const unlockedVendorVault = mainstay.web3.Keypair.generate();
  ...
  it("Drops an unlocked reward", async () => {
    ...
    const rewardAmount = new mainstay.BN(200);
    const expiry = new mainstay.BN(Date.now() / 1000 + 5);
    ...
    assert.isTrue(vendorAccount.poolTokenSupply.eq(new mainstay.BN(10)));
  ...
  it("Collects an unlocked reward", async () => {
    ...
    assert.isTrue(tokenAccount.amount.eq(new mainstay.BN(200)));

    const memberAccount = await registry.account.member.fetch(member.publicKey);
    assert.strictEqual(memberAccount.rewardsCursor, 1);
  });

  const lockedVendor = mainstay.web3.Keypair.generate();
  const lockedVendorVault = mainstay.web3.Keypair.generate();
  ...
  it("Drops a locked reward", async () => {
    lockedRewardKind = {
      locked: {
        startTs: new mainstay.BN(Date.now() / 1000),
        endTs: new mainstay.BN(Date.now() / 1000 + 6),
        periodCount: new mainstay.BN(2),
      },
    };
    lockedRewardAmount = new mainstay.BN(200);
    const expiry = new mainstay.BN(Date.now() / 1000 + 5);
    const [_vendorSigner, nonce] =
      await mainstay.web3.PublicKey.findProgramAddress(
  ...
  it("Claims a locked reward", async () => {
    vendoredVesting = mainstay.web3.Keypair.generate();
    vendoredVestingVault = mainstay.web3.Keypair.generate();
    ...
    assert.isTrue(lockupAccount.whitelistOwned.eq(new mainstay.BN(0)));
  ...
  it("Should fail to unlock an unrealized lockup reward", async () => {
    ...
    await nativeAssert.rejects(
      async () => {
        const withdrawAmount = new mainstay.BN(10);
  ...
  const pendingWithdrawal = mainstay.web3.Keypair.generate();

  it("Unstakes (unlocked)", async () => {
    const unstakeAmount = new mainstay.BN(10);
    ...
    assert.isTrue(vaultPw.amount.eq(new mainstay.BN(20)));
    assert.isTrue(vaultStake.amount.eq(new mainstay.BN(0)));
    assert.isTrue(spt.amount.eq(new mainstay.BN(0)));
  });
  ...
  it("Unstake finalizes (unlocked)", async () => {
    ...
    assert.isTrue(vault.amount.eq(new mainstay.BN(120)));
    assert.isTrue(vaultPw.amount.eq(new mainstay.BN(0)));
  });
  ...
  it("Withdraws deposits (unlocked)", async () => {
    ...
    const withdrawAmount = new mainstay.BN(100);
  ...
  it("Should successfully unlock a locked reward after unstaking", async () => {
    ...
    const withdrawAmount = new mainstay.BN(7);
    await lockup.rpc.withdraw(withdrawAmount, {
      accounts: {
        vesting: vendoredVesting.publicKey,
        beneficiary: provider.wallet.publicKey,
