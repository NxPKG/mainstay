...
/** Command line argument for Mainstay version */
export const MAINSTAY_VERSION_ARG = "--mainstay-version";
        ...
        if (process.env.CI) {
          throw new Error(
            [
              ...
              "Run `mainstay test --skip-lint` in tests/bench and commit the changes.",
            ].join(" ")
          );
        }
...
/**
 * Get Mainstay version from the passed arguments.
 *
 * Defaults to `unreleased`.
 */
export const getVersionFromArgs = () => {
  const args = process.argv;
  const mainstayVersionArgIndex = args.indexOf(MAINSTAY_VERSION_ARG);
  return mainstayVersionArgIndex === -1
    ? "unreleased"
    : (args[mainstayVersionArgIndex + 1] as Version);
};
...
/** Run `mainstay test` command. */
export const runMainstayTest = () => spawn("mainstay", ["test", "--skip-lint"]);
