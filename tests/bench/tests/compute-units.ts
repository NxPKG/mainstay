...
import * as token from "@nx-pkg/spl-token";
...
describe("Compute units", () => {
  ...
  const program = mainstay.workspace.Bench as mainstay.Program<Bench>;
  const owner = program.provider.publicKey!;

  let mintPk: mainstay.web3.PublicKey;
  let tokenPk: mainstay.web3.PublicKey;
  ...
  const measureComputeUnits = async (
    ...
    options?: Partial<{
      ...
      generateKeypair: (accountName: string) => mainstay.web3.Keypair;
      generatePublicKey: (accountName: string) => mainstay.web3.PublicKey;
    }>
  ) => {
    options ??= {};
    options.accountCounts ??= [1, 2, 4, 8];
    options.generateKeypair ??= () => mainstay.web3.Keypair.generate();
    ...
    for (const accountCount of options.accountCounts) {
      ...
      const accounts: { [key: string]: mainstay.web3.PublicKey } = {};
      const signers = [];
      ...
      for (const ixName of ixNames) {
        ...
        for (const ix of program.idl.instructions) {
          ...
          for (const account of ix.accounts) {
            ...
            // Skip other accounts to not override Mainstay defaults
  ...
  before(async () => {
    ...
    const tokenProgram = token.splTokenProgram({
      provider: mainstay.MainstayProvider.local(),
    });

    const tx = new mainstay.web3.Transaction();

    // Create mint account
    const mintKp = new mainstay.web3.Keypair();
    ...
    const tokenKp = new mainstay.web3.Keypair();
  ...
  it("Program", async () => {
    await measureComputeUnits("program", {
      generatePublicKey: () => mainstay.web3.SystemProgram.programId,
    });
  });
