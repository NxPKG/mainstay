...
describe("validator-clone", () => {
  ...
  const program = mainstay.workspace.ValidatorClone as Program<ValidatorClone>;
  ...
  it("Cloned non-executable account", async () => {
    ...
    const [accountInfo] = await mainstay.utils.rpc.getMultipleAccounts(
      connection,
      [new mainstay.web3.PublicKey(account)]
    );
  ...
  it("Cloned bpf3-program accounts and their program data", async () => {
    ...
    const accountInfos = await mainstay.utils.rpc.getMultipleAccounts(
      connection,
      accounts.map((acc) => new mainstay.web3.PublicKey(acc))
    );
