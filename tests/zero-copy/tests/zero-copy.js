...
const BN = mainstay.BN;

describe("zero-copy", () => {
  // Configure the client to use the local cluster.
  mainstay.setProvider(mainstay.MainstayProvider.env());

  const program = mainstay.workspace.ZeroCopy;
  const programCpi = mainstay.workspace.ZeroCpi;

  const foo = mainstay.web3.Keypair.generate();
  ...
  const eventQ = mainstay.web3.Keypair.generate();
