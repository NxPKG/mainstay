...
//
// 1. MainstaySerialize and MainstayDeserialize must be derived. Mainstay requires
//    *all* instructions to implement the MainstaySerialize and MainstayDeserialize
//    traits, so any types in method signatures must as well.
// 2. All types for zero copy deserialization are `#[repr(packed)]`. However,
//    the implementation of MainstaySerialize (i.e. borsh), uses references
