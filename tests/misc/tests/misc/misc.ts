...
import { Program, MainstayError, Wallet } from "@nx-pkg/mainstay";
import {
  PublicKey,
  Keypair,
  SystemProgram,
...
const utf8 = mainstay.utils.bytes.utf8;
...
const miscTest = (
  program: mainstay.Program<Misc> | mainstay.Program<MiscOptional>
) => {
  return () => {
    ...
    describe("Data Account", () => {
      ...
      const udata = new mainstay.BN(1);
      const idata = new mainstay.BN(2);
      ...
      it("Can close an account", async () => {
        ...
        const transferIx = mainstay.web3.SystemProgram.transfer({
          fromPubkey: provider.wallet.publicKey,
          toPubkey: data.publicKey,
          lamports: openAccountBalance,
        });
        const transferTransaction = new mainstay.web3.Transaction().add(
          transferIx
        );
      ...
      it("Can close an account twice", async () => {
        const data = mainstay.web3.Keypair.generate();
        await program.methods
          .initialize(new mainstay.BN(10), new mainstay.BN(10))
          .accounts({ data: data.publicKey })
          .preInstructions([await program.account.data.createInstruction(data)])
          .signers([data])
          .rpc();
    ...
    it("Can embed programs into genesis from the Mainstay.toml", async () => {
      const pid = new mainstay.web3.PublicKey(
        "FtMNMKp9DZHKWUyVAsj3Q5QV8ow4P3fUPP7ZrWEQJzKr"
      );
      let accInfo = await mainstay.getProvider().connection.getAccountInfo(pid);
    ...
    it("Can use instruction data in accounts constraints", async () => {
      ...
      const [myPda, nonce] = await PublicKey.findProgramAddress(
        [seed, mainstay.web3.SYSVAR_RENT_PUBKEY.toBuffer()],
        program.programId
      );
    ...
    it("Can create a PDA account with instruction data", async () => {
      ...
      const foo = mainstay.web3.SYSVAR_RENT_PUBKEY;
      const [myPda, nonce] = await PublicKey.findProgramAddress(
        [
          Buffer.from(mainstay.utils.bytes.utf8.encode("my-seed")),
          Buffer.from(mainstay.utils.bytes.utf8.encode(domain)),
          foo.toBuffer(),
          seed,
        ],
        program.programId
    ...
    it("Can create a token account from seeds pda", async () => {
      const [mint, mint_bump] = await PublicKey.findProgramAddress(
        [Buffer.from(mainstay.utils.bytes.utf8.encode("my-mint-seed"))],
        program.programId
      );
      const [myPda, token_bump] = await PublicKey.findProgramAddress(
        [Buffer.from(mainstay.utils.bytes.utf8.encode("my-token-seed"))],
        program.programId
      );
    ...
    it("Can execute a fallback function", async () => {
      await nativeAssert.rejects(
        async () => {
          await mainstay.utils.rpc.invoke(program.programId);
        },
        (err) => {
    ...
    it("Can init a random account prefunded", async () => {
      ...
      await program.rpc.testInit({
        ...
        signers: [data],
        instructions: [
          mainstay.web3.SystemProgram.transfer({
            fromPubkey: provider.wallet.publicKey,
            toPubkey: data.publicKey,
            lamports: 4039280,
          }),
    ...
    it("Can create a random mint account", async () => {
      mint = mainstay.web3.Keypair.generate();
    ...
    it("Can create a random token account", async () => {
      const token = mainstay.web3.Keypair.generate();
    ...
    it("Can initialize multiple accounts via a composite payer", async () => {
      const data1 = mainstay.web3.Keypair.generate();
      const data2 = mainstay.web3.Keypair.generate();
    ...
    it("Can create a random mint account with token program", async () => {
      const newMint = mainstay.web3.Keypair.generate();
    ...
    it("Can create a random token account with token program", async () => {
      ...
      assert.strictEqual(new mainstay.BN(ataAccount.amount).toNumber(), 0);
      assert.strictEqual(
    ...
    describe("associated_token constraints", () => {
      ...
      it("Can use fetchNullable() on accounts with only a balance", async () => {
        const account = mainstay.web3.Keypair.generate();

        // Airdrop 1 SOL to the account.
        const signature = await program.provider.connection.requestAirdrop(
          account.publicKey,
          mainstay.web3.LAMPORTS_PER_SOL
        );
      ...
      it("associated_token constraints check do not allow authority change", async () => {
        ...
        await localClient.setAuthority(
          associatedToken,
          mainstay.web3.Keypair.generate().publicKey,
          "AccountOwner",
          wallet.payer,
          []
        );
      ...
      it("associated_token constraints (no init) - Can make with associated_token::token_program", async () => {
        const mint = mainstay.web3.Keypair.generate();
        await program.rpc.testInitMintWithTokenProgram({
          accounts: {
            mint: mint.publicKey,
            payer: provider.wallet.publicKey,
    ...
    it("Can fetch all accounts of a given type", async () => {
      ...
      const data1 = mainstay.web3.Keypair.generate();
      const data2 = mainstay.web3.Keypair.generate();
      const data3 = mainstay.web3.Keypair.generate();
      const data4 = mainstay.web3.Keypair.generate();
      // Initialize filterable data.
      const filterable1 = mainstay.web3.Keypair.generate().publicKey;
      const filterable2 = mainstay.web3.Keypair.generate().publicKey;
      // Set up a secondary wallet and program.
      const anotherProvider = new mainstay.MainstayProvider(
        program.provider.connection,
        new mainstay.Wallet(mainstay.web3.Keypair.generate()),
        { commitment: program.provider.connection.commitment }
      );

      const anotherProgram = new mainstay.Program(
    ...
    it("Can use pdas with empty seeds", async () => {
      ...
      const [pda2] = await PublicKey.findProgramAddress(
        [mainstay.utils.bytes.utf8.encode("non-empty")],
        program.programId
      );
      await nativeAssert.rejects(
    ...
    const ifNeededAcc = mainstay.web3.Keypair.generate();
    ...
    it("init_if_needed creates token account if not exists", async () => {
      ...
      const newToken = mainstay.web3.Keypair.generate();
    ...
    it("init_if_needed throws if account exists but is not owned by the expected program", async () => {
      const newAcc = await mainstay.web3.PublicKey.findProgramAddress(
        [utf8.encode("hello")],
        program.programId
      );
      ...
      try {
        await program.rpc.testInitIfNeededChecksOwner({
          accounts: {
            ...
            systemProgram: mainstay.web3.SystemProgram.programId,
            payer: provider.wallet.publicKey,
            owner: mainstay.web3.Keypair.generate().publicKey,
          },
        });
        expect(false).to.be.true;
      } catch (_err) {
    ...
    it("init_if_needed throws if pda account exists but does not have the expected seeds", async () => {
      ...
      mainstay.web3.PublicKey.createProgramAddress(
        [utf8.encode("hello")],
        program.programId
      );
    ...
    it("init_if_needed throws if account exists but is not the expected space", async () => {
      const newAcc = mainstay.web3.Keypair.generate();
      ...
      try {
        await program.rpc.testInitMintIfNeeded(6, {
          accounts: {
            ...
            systemProgram: mainstay.web3.SystemProgram.programId,
            tokenProgram: TOKEN_PROGRAM_ID,
            mintAuthority: mainstay.web3.Keypair.generate().publicKey,
            freezeAuthority: provider.wallet.publicKey,
          },
          signers: [mint],
        });
      ...
      try {
        await program.rpc.testInitMintIfNeeded(6, {
          accounts: {
            ...
            tokenProgram: TOKEN_PROGRAM_ID,
            mintAuthority: provider.wallet.publicKey,
            freezeAuthority: mainstay.web3.Keypair.generate().publicKey,
          },
          signers: [mint],
        });
        expect(false).to.be.true;
      ...
      try {
        await program.rpc.testInitTokenIfNeeded({
          accounts: {
            ...
            systemProgram: mainstay.web3.SystemProgram.programId,
            tokenProgram: TOKEN_PROGRAM_ID,
            authority: mainstay.web3.Keypair.generate().publicKey,
          },
          signers: [token],
        });
        expect(false).to.be.true;
    ...
    it("init_if_needed throws if token exists but has the wrong mint", async () => {
      ...
      const mint2 = mainstay.web3.Keypair.generate();
      await program.rpc.testInitMint({
        accounts: {
      ...
      it("With bumps using create_program_address", async () => {
        const [firstPDA, firstBump] =
          await mainstay.web3.PublicKey.findProgramAddress(
            [mainstay.utils.bytes.utf8.encode("seed")],
            ASSOCIATED_TOKEN_PROGRAM_ID
          );
        ...
        const wrongAddress = mainstay.web3.Keypair.generate().publicKey;
        try {
    ...
    describe("Token Constraint Test", () => {
      ...
      it("Token Constraint Test(no init) - throws if token::mint mismatch", async () => {
        ...
        const mint1 = mainstay.web3.Keypair.generate();
        await program.rpc.testInitMint({
          accounts: {
...
describe("misc", miscTest(mainstay.workspace.Misc as Program<Misc>));

describe(
  "misc-optional",
  miscTest(mainstay.workspace.MiscOptional as Program<MiscOptional>)
);
