...
describe("New IDL", () => {
  ...
  const program = mainstay.workspace.newIdl as mainstay.Program<NewIdl>;

  describe("Case conversion", () => {
    const caseConversionAccountKp = mainstay.web3.Keypair.generate();
    const FIELD_NAME = 5;
  ...
  describe("Client interaction", () => {
    ...
    it("Can use primitive types", async () => {
      const kp = mainstay.web3.Keypair.generate();
      ...
      const i16 = 1;
      const i32 = -5555551;
      const i64 = new mainstay.BN("384535471");
      const i128 = new mainstay.BN(-8342491);

      const u8 = 123;
      const u16 = 7888;
      const u32 = 5555551;
      const u64 = new mainstay.BN("384535471");
      const u128 = new mainstay.BN(8888888);

      const f32 = 1.0;
      const f64 = 0.618;

      const pubkey = mainstay.web3.PublicKey.default;
    ...
    it("Can use struct", async () => {
      ...
      const namedStructArg = {
        ...
        u16: 11,
        u32: 111,
        u64: new mainstay.BN(1111),
      } as const;
      const tupleStructArg = [new mainstay.BN(23), "tuple"] as const;
    ...
    it("Can use enum", async () => {
      ...
      // Named
      const pointX = new mainstay.BN(1);
      const pointY = new mainstay.BN(2);
      ...
      const tupleStructArg = [
        { u8: 1, u16: 11, u32: 111, u64: new mainstay.BN(1111) },
      ] as const;
    ...
    it("Can use type aliases", async () => {
      ...
      const aliasStruct = {
        ...
        u16: 2,
        u32: 3,
        u64: new mainstay.BN(4),
      };
      const aliasVecString = ["first", "second"];
      const aliasOptionVecPubkey = [mainstay.web3.Keypair.generate().publicKey];
      const aliasGenericConst = [1, 23045, 32, 4];
      const aliasMultipleGenericsMixed = [
        [true, false],
        [false, true],
      ];
      const aliasExternal = new mainstay.BN(1708705033);
    ...
    it("Can use full module path types", async () => {
      ...
      const namedStructArg = { u8: 1, u16: 2, u32: 3, u64: new mainstay.BN(4) };
    ...
    it("Can use non-Mainstay external types", async () => {
      const feature = { activatedAt: new mainstay.BN(42) };

      const kp = mainstay.web3.Keypair.generate();
      await program.methods
        .externalNonMainstay(feature)
        .accounts({ account: kp.publicKey })
        .signers([kp])
        .preInstructions([
          await program.account.accountWithNonMainstayExternalField.createInstruction(
            kp
          ),
        ])
        .rpc();
      ...
      const account =
        await program.account.accountWithNonMainstayExternalField.fetch(
          kp.publicKey
        );
  ...
  describe("Format", () => {
    const ixCoder = new mainstay.BorshInstructionCoder(program.idl);
    ...
    const formatEnum = async (argName: string, data: any, expected: string) => {
      ...
      const ix = await program.methods
        .enm(arg)
        .accounts({ account: mainstay.web3.PublicKey.default })
        .instruction();
    ...
    it("Can format named enum", async () => {
      await formatEnum(
        ...
        {
          fullEnum: {
            named: { pointX: new mainstay.BN(1), pointY: new mainstay.BN(2) },
          },
        },
