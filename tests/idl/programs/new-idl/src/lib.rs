use mainstay_lang::{prelude::*, solana_program::clock::UnixTimestamp};
...
pub mod new_idl {
    ...
    pub fn external_non_mainstay(
        ctx: Context<ExternalNonMainstay>,
...
#[derive(MainstayDeserialize, MainstaySerialize, Clone)]
pub struct UnitStruct;

#[derive(MainstaySerialize, MainstayDeserialize, Clone, Copy, Debug, Eq, PartialEq)]
...
#[derive(MainstaySerialize, MainstayDeserialize, Clone, Debug)]
...
#[derive(Accounts)]
pub struct ExternalNonMainstay<'info> {
    #[account(zero)]
    pub account: Account<'info, AccountWithNonMainstayExternalField>,
}

#[account]
pub struct AccountWithNonMainstayExternalField {
    pub feature: wrapped::Feature,
}

/// An example of wrapping a non-Mainstay external type in order to include it in the IDL
mod wrapped {
    use super::*;

    #[cfg(feature = "idl-build")]
    use mainstay_lang::idl::types::*;

    pub struct Feature(mainstay_lang::solana_program::feature::Feature);

    impl MainstaySerialize for Feature {
    ...
    impl MainstayDeserialize for Feature {
        fn deserialize_reader<R: std::io::prelude::Read>(reader: &mut R) -> std::io::Result<Self> {
            Ok(Self(mainstay_lang::solana_program::feature::Feature {
                activated_at: MainstayDeserialize::deserialize_reader(reader)?,
            }))
        }
    }
    ...
    impl Clone for Feature {
        fn clone(&self) -> Self {
            Self(mainstay_lang::solana_program::feature::Feature {
