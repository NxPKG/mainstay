...
describe("Optional", () => {
  ...
  const mainstayProvider = mainstay.MainstayProvider.env();
  const program = mainstay.workspace.Optional as Program<Optional>;
  ...
  const payerWallet = (program.provider as mainstay.MainstayProvider).wallet;
  ...
  describe("Missing optionals feature tests", async () => {
    it("Fails with missing optional accounts at the end by default", async () => {
      ...
      try {
        ...
        // @ts-ignore
        assert.isTrue(e instanceof MainstayError, e.toString());
        const err: MainstayError = <MainstayError>e;
    ...
    it("Succeeds with missing optional accounts at the end with the feature on", async () => {
      const allowMissingOptionals = mainstay.workspace
      ...
      await mainstayProvider.sendAndConfirm(doStuffTxn);
    });
  });

  describe("Initialize tests", async () => {
    it("Initialize with required null fails mainstay-ts validation", async () => {
      const [requiredKeypair, createRequiredIx] = await createRequired();
      try {
  ...
  describe("Update tests", async () => {
    ...
    it("Fails with a missing signer", async () => {
      try {
        ...
        await mainstayProvider.sendAndConfirm(txn);
        assert.fail(
        ...
        assert.strictEqual(
          mainstayError.error.errorMessage,
          LangErrorMessage.get(errorCode)
        );
        assert.strictEqual(mainstayError.error.errorCode.number, errorCode);
      }
    });
