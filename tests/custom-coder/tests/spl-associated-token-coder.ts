import { Native, MainstayProvider, setProvider } from "@nx-pkg/mainstay";
import { splAssociatedTokenAccountProgram } from "@nx-pkg/spl-associated-token-account";
import { splTokenProgram } from "@nx-pkg/spl-token";
import { Keypair, PublicKey, SYSVAR_RENT_PUBKEY } from "@solana/web3.js";
import * as assert from "assert";

describe("spl-associated-token-coder", () => {
  // Configure the client to use the local cluster.
  const provider = MainstayProvider.env();
