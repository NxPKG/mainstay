...
Adds a directory where you want the `<idl>.ts` file to be copied when running `mainstay build`. This is helpful when you want to keep this file in version control, like when using it on the frontend, which will probably not have access to the `target` directory generated by mainstay.
...
```toml
[toolchain]
mainstay_version = "0.30.1"    # `mainstay-cli` version to use(requires `avm`)
