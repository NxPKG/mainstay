...
description: Mainstay - Milestone Project - Tic-Tac-Toe
---

> [Program Code](https://github.com/nxpkg/mainstay/tree/master/docs/programs/tic-tac-toe)

You're now ready to build your first mainstay project. Create a new mainstay workspace with

```shell
mainstay init tic-tac-toe
```
...
Let us briefly explain how we arrived at the `Game::MAXIMUM_SIZE`. Mainstay uses the [borsh](https://borsh.io) specification to (de)serialize its state accounts.
...
In addition to the game's size, we have to add another 8 to the space. This is space for the internal discriminator which mainstay sets automatically. In short, the discriminator is how mainstay can differentiate between different accounts of the same program. For more information, check out the Mainstay space reference.

> [Mainstay Space Reference](./../mainstay_references/space.md)
...
And with this, `SetupGame` is complete and we can move on to the `setup_game` function. (If you like playing detective, you can pause here and try to figure out why what we just did will not work. Hint: Have a look at the [specification](https://borsh.io/) of the serialization library Mainstay uses. If you cannot figure it out, don't worry. We are going to fix it very soon, together.)
...
Now, run `mainstay build`. On top of compiling your program, this command creates an [IDL](https://en.wikipedia.org/wiki/Interface_description_language) for your program. You can find it in `target/idl`. The mainstay typescript client can automatically parse this IDL and generate functions based on it. What this means is that each mainstay program gets its own typescript client for free! (Technically, you don't have to call `mainstay build` before testing. `mainstay test` will do it for you.)
...
```typescript
it('setup game!', async () => {
  const gameKeypair = mainstay.web3.Keypair.generate()
  const playerOne = (program.provider as mainstay.MainstayProvider).wallet
  const playerTwo = mainstay.web3.Keypair.generate()
...
> To generate them, run `mainstay build`. This builds the program and creates the idl and typescript types.

The test begins by creating some keypairs. Importantly, `playerOne` is not a keypair but the wallet of the program's provider. The provider details are defined in the `Mainstay.toml` file in the root of the project. The provider serves as the keypair that pays for (and therefore signs) all transactions.
...
We did not have to specify the `system_program` account. This is because mainstay recognizes this account and is able to infer it. This is also true for other known accounts such as the `token_program` or the `rent` sysvar account.
...
Finally, we verify the game has been set up properly by comparing the actual state and the expected state. To learn how Mainstay maps the Rust types to the js/ts types, check out the [Javascript Mainstay Types Reference](./../mainstay_references/javascript_mainstay_types_reference.md).

Now, run `mainstay test`. This starts up (and subsequently shuts down) a local validator (make sure you don't have one running before) and runs your tests using the test script defined in `Mainstay.toml`.
...
async function play(
  ...
  await program.methods
    ...
    .signers(player instanceof (mainstay.Wallet as any) ? [] : [player])
    .rpc()
...
and run `mainstay test`.

You can finish writing the test by yourself (or check out [the reference implementation](https://github.com/nxpkg/mainstay/tree/master/docs/programs/tic-tac-toe)). Try to simulate a win and a tie!

Proper testing also includes tests that try to exploit the contract. You can check whether you've protected yourself properly by calling `play` with unexpected parameters. You can also familiarize yourself with the returned `MainstayErrors` this way. For example:
...
try {
  ...
  expect(_err).to.be.instanceOf(MainstayError)
...
1. Run `mainstay build`. Your program keypair is now in `target/deploy`. Keep this keypair secret. You can reuse it on all clusters.
2. Run `mainstay keys list` to display the keypair's public key and copy it into your `declare_id!` macro at the top of `lib.rs`.
3. Run `mainstay build` again. This step is necessary to include the new program id in the binary.
4. Change the `provider.cluster` variable in `Mainstay.toml` to `devnet`.
5. Run `mainstay deploy`
6. Run `mainstay test`
...
Well done! You've finished the essentials section. You can now move on to the more advanced parts of Mainstay.
