...
description: Mainstay - Program Derived Addresses
---
...
Before we dive into how to use PDAs in mainstay, here's a short explainer on what PDAs are.
...
We are now going to show you what you can do with PDAs and how to do it in Mainstay!

### Hashmap-like structures using PDAs

Before we dive into the specifics of creating hashmaps in mainstay, let's look at how to create a hashmap with PDAs in general.
...
#### How to build PDA hashmaps in Mainstay
...
Additionally, we add an empty `bump` constraint to signal to mainstay that it should find the canonical bump itself.
Then, in the handler, we call `ctx.bumps.user_stats` to get the bump mainstay found and save it to the user stats
...
describe('game', async () => {
  ...
  const program = mainstay.workspace.Game as Program<Game>

  it('Sets and changes name!', async () => {
    const [userStatsPDA, _] = await PublicKey.findProgramAddress(
      [
        mainstay.utils.bytes.utf8.encode('user-stats'),
...
In mainstay, to sign with a pda you have to change `CpiContext::new(cpi_program, cpi_accounts)` to `CpiContext::new_with_signer(cpi_program, cpi_accounts, seeds)` where the `seeds` argument are the seeds _and_ the bump the PDA was created with.
...
describe('puppet', () => {
  ...
  it('Does CPI!', async () => {
    ...
    await puppetMasterProgram.methods
      .pullStrings(puppetMasterBump, new mainstay.BN(42))
      .accounts({
