...
description: Mainstay - Release Notes 0.30.0
---

The long-awaited v0.30.0 release is finally here!

We'll go over the main changes, but if you'd like to see all notable changes, check out the [CHANGELOG](https://github.com/nxpkg/mainstay/blob/v0.30.0/CHANGELOG.md#0300---2024-04-15).
...
1. Update `avm`:
   ...
   ```sh
   cargo install --git https://github.com/nxpkg/mainstay --tag v0.30.0 avm --locked
   ```
...
3. Update Mainstay crate(s) to `0.30.0`. Optionally, run `cargo update` to update other dependencies to the latest compatible versions.
...
The IDL type specification and generation has been rewritten. To keep the release notes short, we won't go over the changes here, but see [this](https://github.com/nxpkg/mainstay/pull/2824) if you'd like to learn more.

### `idl-build` feature

`idl-build` feature is now required in your program's `Cargo.toml` definition in order for the IDL generation to work.

Without this feature, `mainstay build` outputs:
...
Note that all crates that you use to generate type definitions for the IDL need to be specified in the list of `idl-build`, e.g. `mainstay-spl/idl-build`, `some-program/idl-build`...

## Lang

### Dependency free program declaration

Depending on other crates who used different versions of Mainstay is not the best experience, to say the least. To solve this problem, program clients can now be generated from their IDL using the new `declare_program!` macro:
...
This works for both on-chain (CPI) and off-chain (RPC) usage, allowing program interactions without creating a [dependency hell](https://en.wikipedia.org/wiki/Dependency_hell). Check out [this](https://github.com/nxpkg/mainstay/blob/v0.30.0/tests/declare-program/programs/declare-program/src/lib.rs) example for on-chain CPI usage.

For more information, see the macro's [documentation](https://docs.rs/mainstay-lang/0.30.0/mainstay_lang/macro.declare_program.html).
...
**Note:** Above values are concatinated with `::` (similar to other Mainstay constraints) and have `extensions` prefix e.g. `extensions::group_pointer::authority = <EXPR>`.

These constraints can be used both with or without the `init` constraint.

[Here](https://github.com/nxpkg/mainstay/blob/v0.30.0/tests/spl/token-extensions/programs/token-extensions/src/instructions.rs) is an example program that uses these constraints.

#### CPI wrappers

`mainstay-spl` now includes CPI wrappers for Token Extensions which can be accessed from `mainstay_spl::token_2022_extensions`.

### `#[interface]` attribute

Transfer hooks can now be used with the new `#[interface]` macro. This argument overrides the Mainstay's default instruction discriminator to use the interface instruction's discriminator.
...
```
mainstay idl erase-authority --program-id <PROGRAM_ID> --priority-fee 9000
```
...
You can now pass arguments to `solana program deploy` from `mainstay deploy`:

```
mainstay deploy -- --final
```
...
```
mainstay build -p my-program
```

### Deactivate test-validator features

You can now deactivate test-validator features from `Mainstay.toml`:
...
Using non-matching versions of `mainstay-cli`, `mainstay-lang`, and `@nx-pkg/mainstay` can result in unexpected behavior. In this release, you'll get a warning if any of them don't match.

### Explicit `overflow-checks` flag

[`overflow-checks`](https://doc.rust-lang.org/cargo/reference/profiles.html#overflow-checks) flag is implicitly disabled by default. Mainstay workspaces that are crated with `mainstay init` have this flag enabled, however, Mainstay doesn't do any checks for it after the initial workspace creation.
...
### Wildcard pattern in `Mainstay.toml`
...
Before this release, `mainstay build` used `cargo build-bpf` to build programs, however, because it is deprecated, `mainstay build` now defaults to `cargo build-sbf`.

To preserve the old behavior, you can use:

```
mainstay build --arch bpf
```

### Run multiple commands in scripts

Scripts in `Mainstay.toml` now supports running multiple commands:
...
```
mainstay test --program-name example
```
...
```
mainstay init --test-template rust
```
...
The internals of the TS library are filled with case conversion logic before making string comparison and this also forces other libraries who build on top of Mainstay to do the same.
...
`opts` parameter of `MainstayProvider` is now optional:

```diff
- new MainstayProvider(connection, wallet, {});
+ new MainstayProvider(connection, wallet);
```

### Type changes

There are too many type changes to list here, especially the types that are related to the IDL. The new IDL types can be found [here](https://github.com/nxpkg/mainstay/blob/v0.30.0/ts/packages/mainstay/src/idl.ts).

---

See the full list of notable changes in the [CHANGELOG](https://github.com/nxpkg/mainstay/blob/v0.30.0/CHANGELOG.md#0300---2024-04-15).
