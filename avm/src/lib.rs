...
pub fn version_binary_path(version: &Version) -> PathBuf {
    get_bin_dir_path().join(format!("mainstay-{version}"))
}
...
pub fn use_version(opt_version: Option<Version>) -> Result<()> {
    let version = match opt_version {
        ...
        None => read_mainstayversion_file()?,
    };
    ...
    println!("Now using mainstay version {}.", current_version()?);
    Ok(())
}
...
fn get_mainstay_version_from_commit(commit: &str) -> Result<Version> {
    ...
    if response.status() != StatusCode::OK {
        return Err(anyhow!(
            "Could not find mainstay-cli version for commit: {response:?}"
        ));
    };
...
/// Install a version of mainstay-cli
pub fn install_version(install_target: InstallTarget, force: bool) -> Result<()> {
    ...
    let version = match install_target {
        ...
        InstallTarget::Commit(commit) => {
            ...
            get_mainstay_version_from_commit(&commit)?
        }
    };
...
/// Remove an installed version of mainstay-cli
pub fn uninstall_version(version: &Version) -> Result<()> {
    let version_path = get_bin_dir_path().join(format!("mainstay-{version}"));
    if !version_path.exists() {
        return Err(anyhow!("mainstay-cli {} is not installed", version));
    }
    if version == &current_version()? {
        return Err(anyhow!("mainstay-cli {} is currently in use", version));
    }
...
/// Read version from .mainstayversion
pub fn read_mainstayversion_file() -> Result<Version> {
    fs::read_to_string(".mainstayversion")
        .map_err(|e| anyhow!(".mainstayversion file not found: {e}"))
        .map(|content| Version::parse(content.trim()))?
        .map_err(|e| anyhow!("Unable to parse version: {e}"))
}

/// Retrieve a list of installable versions of mainstay-cli using the GitHub API and tags on the Mainstay
/// repository.
...
/// Read the installed mainstay-cli versions by reading the binaries in the AVM_HOME/bin directory.
pub fn read_installed_versions() -> Result<Vec<Version>> {
    const PREFIX: &str = "mainstay-";
...
mod tests {
    ...
    fn test_version_binary_path() {
        assert_eq!(
            ...
            get_bin_dir_path().join("mainstay-0.18.2")
        );
    }

    #[test]
    fn test_read_mainstayversion() -> Result<()> {
        ensure_paths();

        let mainstayversion_path = Path::new(".mainstayversion");
        let test_version = "0.26.0";
        fs::write(mainstayversion_path, test_version)?;

        let version = read_mainstayversion_file()?;
        assert_eq!(version.to_string(), test_version);

        fs::remove_file(mainstayversion_path)?;
    ...
    #[test]
    #[should_panic(expected = "mainstay-cli 0.18.1 is not installed")]
    fn test_uninstall_non_installed_version() {
        uninstall_version(&Version::parse("0.18.1").unwrap()).unwrap();
    }

    #[test]
    #[should_panic(expected = "mainstay-cli 0.18.2 is currently in use")]
    fn test_uninstalled_in_use_version() {
        ...
        // mitigate the read not seeing the written version bytes.
        current_version_file.sync_all().unwrap();
        // Create a fake binary for mainstay-0.18.2 in the bin directory
    ...
    fn test_read_installed_versions() {
        ...
        // Should ignore this file because its not mainstay- prefixed
        fs::File::create(AVM_HOME.join("bin").join("garbage").as_path()).unwrap();
        assert_eq!(read_installed_versions().unwrap(), expected);
    }

    #[test]
    fn test_get_mainstay_version_from_commit() {
        let version =
            get_mainstay_version_from_commit("e1afcbf71e0f2e10fae14525934a6a68479167b9").unwrap();
        assert_eq!(
            version.to_string(),
