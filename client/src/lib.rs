...
//! An RPC client to interact with Solana programs written in [`mainstay_lang`].
//!
//! # Examples
//!
...
//!
//! [here]: https://github.com/nxpkg/mainstay/tree/v0.30.0/client/example/src
//!
//! # Features
//!
...
//!
//! ```toml
//! mainstay-client = { version = "0.30.0 ", features = ["async"] }
//! ````

use mainstay_lang::solana_program::hash::Hash;
use mainstay_lang::solana_program::instruction::{AccountMeta, Instruction};
...
/// communicate with Mainstay programs running on a Solana cluster. It's
...
/// Auxiliary data structure to align the types of the Solana CLI utils with Mainstay client.
...
impl<C: Deref<Target = impl Signer> + Clone> Program<C> {
    ...
    async fn on_internal<T: mainstay_lang::Event + mainstay_lang::MainstayDeserialize>(
        &self,
...
/// Function type that accepts solana accounts and returns deserialized mainstay accounts
...
pub fn handle_program_log<T: mainstay_lang::Event + mainstay_lang::MainstayDeserialize>(
        ...
        .or_else(|| l.strip_prefix(PROGRAM_DATA))
    {
        ...
        if disc == T::discriminator() {
            let e: T = mainstay_lang::MainstayDeserialize::deserialize(&mut slice)
...
pub enum ClientError {
    ...
    AccountNotFound,
    #[error("{0}")]
    MainstayError(#[from] mainstay_lang::error::Error),
    #[error("{0}")]
...
fn parse_logs_response<T: mainstay_lang::Event + mainstay_lang::MainstayDeserialize>(
...
mod tests {
    ...
    // Creating a mock struct that implements `mainstay_lang::events`
