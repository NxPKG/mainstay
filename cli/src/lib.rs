...
pub enum Command {
    ...
    /// Starts a node shell with an Mainstay client setup according to the local
    /// config.
    Shell,
    /// Runs the script defined by the current workspace's Mainstay.toml.
    Run {
    ...
    /// Publishes a verified build to the Mainstay registry.
    Publish {
...
pub enum IdlCommand {
    ...
    /// Convert legacy IDLs (pre Mainstay 0.30) to the new IDL spec
    Convert {
...
/// Override the toolchain from `Mainstay.toml`.
///
...
fn override_toolchain(cfg_override: &ConfigOverride) -> Result<RestoreToolchainCallbacks> {
    ...
    if let Some(cfg) = cfg {
        ...
        // Mainstay version override should be handled last
        if let Some(mainstay_version) = &cfg.toolchain.mainstay_version {
            // Mainstay binary name prefix(applies to binaries that are installed via `avm`)
            const MAINSTAY_BINARY_PREFIX: &str = "mainstay-";
            ...
            let current_version = std::env::args()
                ...
                .split_once(MAINSTAY_BINARY_PREFIX)
                .map(|(_, version)| version)
                .unwrap_or(VERSION)
                .to_owned();
            if mainstay_version != &current_version {
                let binary_path = home_dir()
                    .unwrap()
                    .join(".avm")
                    .join("bin")
                    .join(format!("{MAINSTAY_BINARY_PREFIX}{mainstay_version}"));

                if !binary_path.exists() {
                    eprintln!(
                        "`mainstay` {mainstay_version} is not installed with `avm`. Installing...\n"
                    );

                    let exit_status = std::process::Command::new("avm")
                        .arg("install")
                        .arg(mainstay_version)
                        .spawn()?
                        .wait()?;
                    if !exit_status.success() {
                        eprintln!(
                            "Failed to install `mainstay` {mainstay_version}, \
                            using {current_version} instead"
                        );
...
fn init(
    ...
    // Mainstay converts to snake case before writing the program name
    if syn::parse_str::<syn::Ident>(&rust_name).is_err()
        || extra_keywords.contains(&rust_name.as_str())
    {
        return Err(anyhow!(
            "Mainstay workspace name must be a valid Rust identifier. It may not be a Rust reserved word, start with a digit, or include certain disallowed characters. See https://doc.rust-lang.org/reference/identifiers.html for more detail.",
        ));
    }
...
fn expand_program(
    ...
    if !exit.status.success() {
        eprintln!("'mainstay expand' failed. Perhaps you have not installed 'cargo-expand'? https://github.com/dtolnay/cargo-expand#installation");
...
fn build_all(
    ...
    let r = match cfg_path.parent() {
        None => Err(anyhow!("Invalid Mainstay.toml at {}", cfg_path.display())),
        Some(_parent) => {
...
// Builds an mainstay program in a docker image and copies the build artifacts
...
fn build_cwd_verifiable(
    ...
    let container_name = "mainstay-program";
...
fn idl_set_buffer(
    ...
    with_workspace(cfg_override, |cfg| {
        ...
        let ix = {
            ...
            let mut data = mainstay_lang::idl::IDL_IX_TAG.to_le_bytes().to_vec();
...
fn idl_set_authority(
    ...
    with_workspace(cfg_override, |cfg| {
        ...
        // Instruction data.
        let data =
            serialize_idl_ix(mainstay_lang::idl::IdlInstruction::SetAuthority { new_authority })?;
...
fn idl_close_account(
    ...
    let ix = Instruction {
        ...
        accounts,
        data: { serialize_idl_ix(mainstay_lang::idl::IdlInstruction::Close {})? },
    };
...
fn idl_write(
    ...
    while offset < idl_data.len() {
        ...
        let data = {
            ...
            serialize_idl_ix(mainstay_lang::idl::IdlInstruction::Write {
...
fn deserialize_idl_defined_type_to_json(
    ...
    match defined_type {
        ...
        IdlTypeDefTy::Enum { variants } => {
            let repr = <u8 as MainstayDeserialize>::deserialize(data)?;
...
// Deserializes a primitive type using MainstayDeserialize
fn deserialize_idl_type_to_json(
    ...
    Ok(match idl_type {
        IdlType::Bool => json!(<bool as MainstayDeserialize>::deserialize(data)?),
        IdlType::U8 => {
            json!(<u8 as MainstayDeserialize>::deserialize(data)?)
        }
        IdlType::I8 => {
            json!(<i8 as MainstayDeserialize>::deserialize(data)?)
        }
        IdlType::U16 => {
            json!(<u16 as MainstayDeserialize>::deserialize(data)?)
        }
        IdlType::I16 => {
            json!(<i16 as MainstayDeserialize>::deserialize(data)?)
        }
        IdlType::U32 => {
            json!(<u32 as MainstayDeserialize>::deserialize(data)?)
        }
        IdlType::I32 => {
            json!(<i32 as MainstayDeserialize>::deserialize(data)?)
        }
        IdlType::F32 => json!(<f32 as MainstayDeserialize>::deserialize(data)?),
        IdlType::U64 => {
            json!(<u64 as MainstayDeserialize>::deserialize(data)?)
        }
        IdlType::I64 => {
            json!(<i64 as MainstayDeserialize>::deserialize(data)?)
        }
        IdlType::F64 => json!(<f64 as MainstayDeserialize>::deserialize(data)?),
        IdlType::U128 => {
            // TODO: Remove to_string once serde_json supports u128 deserialization
            json!(<u128 as MainstayDeserialize>::deserialize(data)?.to_string())
        }
        IdlType::I128 => {
            // TODO: Remove to_string once serde_json supports i128 deserialization
            json!(<i128 as MainstayDeserialize>::deserialize(data)?.to_string())
        }
        ...
        IdlType::Bytes => JsonValue::Array(
            <Vec<u8> as MainstayDeserialize>::deserialize(data)?
                .iter()
                .map(|i| json!(*i))
                .collect(),
        ),
        IdlType::String => json!(<String as MainstayDeserialize>::deserialize(data)?),
        IdlType::Pubkey => {
            json!(<Pubkey as MainstayDeserialize>::deserialize(data)?.to_string())
        }
        ...
        IdlType::Option(ty) => {
            let is_present = <u8 as MainstayDeserialize>::deserialize(data)?;
        ...
        IdlType::Vec(ty) => {
            let size: usize = <u32 as MainstayDeserialize>::deserialize(data)?
                .try_into()
                .unwrap();
    ...
    with_workspace(cfg_override, |cfg| {
        ...
        if let Some(test_script) = cfg.scripts.get_mut("test") {
            ...
            match program_name {
                ...
                _ => println!(
                    "\nFound a 'test' script in the Mainstay.toml. Running it as a test suite!"
                ),
            }
...
fn run_test_suite(
    ...
    let test_result = {
        ...
        std::process::Command::new("bash")
            .arg("-c")
            .arg(script_args)
            .env("MAINSTAY_PROVIDER_URL", url)
            .env("MAINSTAY_WALLET", cfg.provider.wallet.to_string())
...
fn start_test_validator(
    ...
    if count >= ms_wait {
        eprintln!(
            ...
            Check {test_ledger_log_filename:?} for errors. Consider increasing [test.startup_wait] in Mainstay.toml."
        );
...
fn create_idl_account(
    ...
    // Run `Create instruction.
    {
        ...
        let data = serialize_idl_ix(mainstay_lang::idl::IdlInstruction::Create { data_len })?;
        ...
        for _ in 0..num_additional_instructions {
            let data = serialize_idl_ix(mainstay_lang::idl::IdlInstruction::Resize { data_len })?;
            instructions.push(Instruction {
                program_id: *program_id,
                accounts: vec![
                    AccountMeta::new(idl_address, false),
...
fn serialize_idl_ix(ix_inner: mainstay_lang::idl::IdlInstruction) -> Result<Vec<u8>> {
    let mut data = Vec::with_capacity(256);
    data.extend_from_slice(&mainstay_lang::idl::IDL_IX_TAG.to_le_bytes());
...
fn migrate(cfg_override: &ConfigOverride) -> Result<()> {
    with_workspace(cfg_override, |cfg| {
        ...
        if !Path::new(".mainstay").exists() {
            fs::create_dir(".mainstay")?;
        }
        std::env::set_current_dir(".mainstay")?;
...
fn set_workspace_dir_or_exit() {
    ...
    match d {
        None => {
            println!("Not in mainstay workspace.");
            std::process::exit(1);
        }
        Some(cfg) => {
            match cfg.path().parent() {
...
fn publish(
    ...
    let mainstay_package = MainstayPackage::from(program_name.clone(), &cfg)?;
    let mainstay_package_bytes = serde_json::to_vec(&mainstay_package)?;
    ...
    let dot_mainstay = workspace_dir.join(".mainstay");
    fs::create_dir_all(&dot_mainstay)?;
    let tarball_filename = dot_mainstay.join(format!("{program_name}.tar.gz"));
    let tar_gz = File::create(&tarball_filename)?;
    let enc = GzEncoder::new(tar_gz, Compression::default());
    let mut tar = tar::Builder::new(enc);

    // Files that will always be included if they exist.
    println!("PACKING: Mainstay.toml");
    tar.append_path("Mainstay.toml")?;
    ...
    let ws_dir = dot_mainstay.join("workspace");
    ...
    let form = Form::new()
        .part("manifest", Part::bytes(mainstay_package_bytes))
        .part("workspace", {
...
fn keys_sync(cfg_override: &ConfigOverride, program_name: Option<String>) -> Result<()> {
    with_workspace(cfg_override, |cfg| {
        ...
        for program in cfg.get_programs(program_name)? {
            ...
            // Handle declaration in Mainstay.toml
            'outer: for programs in cfg.programs.values_mut() {
                for (name, deployment) in programs {
                    ...
                    if deployment.address.to_string() != actual_program_id {
                        println!("Found incorrect program id declaration in Mainstay.toml for the program `{name}`");
...
// workspace directory, i.e., where the `Mainstay.toml` file is located, before
...
fn with_workspace<R>(
    ...
    let mut cfg = Config::discover(cfg_override)
        ...
        .expect("Mainstay.toml must always exist");
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "Mainstay workspace name must be a valid Rust identifier.")]
