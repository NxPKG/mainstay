        ...
        while let Some(cwd) = cwd_opt {
            ...
            for f in fs::read_dir(cwd).with_context(|| {
                ...
                if let Some(filename) = p.file_name().and_then(|name| name.to_str()) {
                    ...
                    if filename == "Mainstay.toml" {
                        mainstay_toml = true;
                    }
                }
            }

            // Not found. Go up a directory level, but don't go up from Mainstay.toml
            if mainstay_toml {
                break;
            }
...
pub struct Config {
    ...
    // "mainstay localnet" only has access to the Mainstay.toml,
...
impl Config {
    ...
    // Climbs each parent directory until we find an Mainstay.toml.
...
// This file needs to have the same (sub)structure as Mainstay.toml
// so it can be parsed as a base test file from an Mainstay.toml
...
#[derive(Debug, Serialize, Deserialize)]
pub struct MainstayPackage {
    pub name: String,
    pub address: String,
    pub idl: Option<String>,
}

impl MainstayPackage {
    pub fn from(name: String, cfg: &WithPath<Config>) -> Result<Self> {
        ...
        let program_details = cfg
            .programs
            .get(cluster)
            .ok_or_else(|| anyhow!("Program not provided in Mainstay.toml"))?
            .get(&name)
            .ok_or_else(|| anyhow!("Program not provided in Mainstay.toml"))?;
