...
async function main() {{
    ...
    const connection = new mainstay.web3.Connection(url, preflightCommitment);
    const wallet = mainstay.Wallet.local();

    const provider = new mainstay.MainstayProvider(connection, wallet, {{
        preflightCommitment,
        commitment: 'recent',
    }});
...
module.exports = async function (provider: mainstay.MainstayProvider) {
  // Configure client to use the provider.
  mainstay.setProvider(provider);
...
pub fn node_shell(
    ...
    let mut eval_string = format!(
        r#"
const mainstay = require('@nx-pkg/mainstay');
const web3 = mainstay.web3;
const PublicKey = mainstay.web3.PublicKey;
const Keypair = mainstay.web3.Keypair;

const __wallet = new mainstay.Wallet(
...
const provider = new mainstay.MainstayProvider(__connection, __wallet, {{
...
"#,
    ...
    for program in programs {
        write!(
            &mut eval_string,
            r#"
mainstay.workspace.{} = new mainstay.Program({}, new PublicKey("{}"), provider);
"#,
...
fn test_initialize() {{
    let program_id = "{0}";
    let mainstay_wallet = std::env::var("MAINSTAY_WALLET").unwrap();
    let payer = read_keypair_file(&mainstay_wallet).unwrap();
