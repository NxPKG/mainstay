...
///
/// - `mainstay-lang` crate version
/// - `@nx-pkg/mainstay` package version
///
/// This function logs warnings in the case of a mismatch.
pub fn check_mainstay_version(cfg: &WithPath<Config>) -> Result<()> {
    ...
    let mismatched_lang_version = cfg
        ...
        .filter_map(|man| man.dependencies.get("mainstay-lang").map(|d| d.to_owned()))
    ...
    if let Some(ver) = mismatched_lang_version {
        eprintln!(
            "WARNING: `mainstay-lang` version({ver}) and the current CLI version({cli_version}) \
                 don't match.\n\n\t\
                 This can lead to unwanted behavior. To use the same CLI version, add:\n\n\t\
                 [toolchain]\n\t\
                 mainstay_version = \"{ver}\"\n\n\t\
                 to Mainstay.toml\n"
        );
    }
    ...
    let mismatched_ts_version = package_json
        .get("dependencies")
        .and_then(|deps| deps.get("@nx-pkg/mainstay"))
    ...
    if let Some(ver) = mismatched_ts_version {
        ...
        eprintln!(
            "WARNING: `@nx-pkg/mainstay` version({ver}) and the current CLI version\
                ({cli_version}) don't match.\n\n\t\
                This can lead to unwanted behavior. To fix, upgrade the package by running:\n\n\t\
                {update_cmd} @nx-pkg/mainstay@{cli_version}\n"
        );
    }
...
/// specified in users' `Cargo.toml` might be incompatible with `mainstay-lang`'s dependency.
/// To fix this and similar problems, users should use the crates exported from `mainstay-lang` or
/// `mainstay-spl` when possible.
    ...
    cfg.get_rust_program_list()?
        ...
        .for_each(|man| {
            eprintln!(
                ...
                `mainstay-lang`.\n\
                `use solana_program` becomes `use mainstay_lang::solana_program`.\n\
                Program name: `{}`\n",
                man.package().name()
            )
        });
...
in `{manifest_path:?}`."#
    ...
    // Check `mainstay-spl`'s `idl-build` feature
    manifest
        .dependencies
        .get("mainstay-spl")
        .and_then(|_| manifest.features.get("idl-build"))
        .map(|feature_list| !feature_list.contains(&"mainstay-spl/idl-build".into()))
        .unwrap_or_default()
        .then(|| {
            eprintln!(
                "WARNING: `idl-build` feature of `mainstay-spl` is not enabled. \
                This is likely to result in cryptic compile errors.\n\n\t\
                To solve, add `mainstay-spl/idl-build` to the `idl-build` feature list:\n\n\t\
                [features]\n\t\
                idl-build = [\"mainstay-spl/idl-build\", ...]\n"
            )
        });
