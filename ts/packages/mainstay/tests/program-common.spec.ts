...
describe("program/common", () => {
  ...
  describe("NodeWallet", () => {
    it("should throw an error when MAINSTAY_WALLET is unset", () => {
      const oldValue = process.env.MAINSTAY_WALLET;
      delete process.env.MAINSTAY_WALLET;

      expect(() => NodeWallet.local()).toThrowError(
        "expected environment variable `MAINSTAY_WALLET` is not set."
      );

      process.env.MAINSTAY_WALLET = oldValue;
    });
  });
});
