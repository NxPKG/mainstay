...
const workspace = new Proxy({} as any, {
  get(workspaceCache: { [key: string]: Program }, programName: string) {
    ...
    if (!_populatedWorkspace) {
      ...
      while (!fs.existsSync(path.join(projectRoot, "Mainstay.toml"))) {
      ...
      if (!fs.existsSync(idlFolder)) {
        throw new Error(`${idlFolder} doesn't exist. Did you use "mainstay build"?`);
      }
      ...
      const mainstayToml = toml.parse(
        fs.readFileSync(path.join(projectRoot, "Mainstay.toml"), "utf-8")
      );
      const clusterId = mainstayToml.provider.cluster;
      if (mainstayToml.clusters && mainstayToml.clusters[clusterId]) {
        attachWorkspaceOverride(
          workspaceCache,
          mainstayToml.clusters[clusterId],
          idlMap
        );
      }
