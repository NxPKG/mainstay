import { ProgramErrorStack, MainstayError } from "../src/error";
...
describe("ProgramErrorStack", () => {
  ...
  it("failed inner ix", () => {
    const logs = [
      ...
      "Program log: MainstayError thrown in programs/switchboard_v2/src/actions/aggregator_save_result_action.rs:235. Error Code: OracleMismatchError. Error Number: 6021. Error Message: An unexpected oracle account was provided for the transaction..",
...
describe("MainstayError", () => {
  it("FileLine MainstayError with Pubkeys", () => {
    const logs = [
    ...
    const mainstayError = MainstayError.parse(logs)!;
    expect(mainstayError.program.toString()).toEqual(
      "SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f"
    );
    expect(mainstayError.error.errorCode).toEqual({
      code: "OracleMismatchError",
      number: 6021,
    });
    expect(mainstayError.error.errorMessage).toEqual(
      "An unexpected oracle account was provided for the transaction."
    );
    expect(mainstayError.error.origin).toEqual({
      file: "programs/switchboard_v2/src/actions/aggregator_save_result_action.rs",
      line: 235,
    });
    expect(
      mainstayError.error.comparedValues!.map((pk) => pk.toString())
    ).toEqual([
      "Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS",
      "SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f",
    ]);
    expect(
      mainstayError.programErrorStack!.map((publicKey) => publicKey.toString())
    ).toEqual(["SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f"]);
    expect(mainstayError.errorLogs).toEqual([
      "Program log: MainstayError thrown in programs/switchboard_v2/src/actions/aggregator_save_result_action.rs:235. Error Code: OracleMismatchError. Error Number: 6021. Error Message: An unexpected oracle account was provided for the transaction..",
      "Program log: Left:",
      "Program log: Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS",
      "Program log: Right:",
  ...
  it("FileLine MainstayError with Values", () => {
    const logs = [
      "Program SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f invoke [1]",
      "Program log: Instruction: AggregatorSaveResult",
      "Program log: MainstayError thrown in programs/switchboard_v2/src/actions/aggregator_save_result_action.rs:235. Error Code: OracleMismatchError. Error Number: 6021. Error Message: An unexpected oracle account was provided for the transaction..",
    ...
    expect(mainstayError.error.comparedValues!).toEqual(["1337", "4220"]);
    expect(
      mainstayError.programErrorStack!.map((publicKey) => publicKey.toString())
    ).toEqual(["SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f"]);
    expect(mainstayError.errorLogs).toEqual([
  ...
  it("AccountName MainstayError with Pubkeys", () => {
    const logs = [
      "Program SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f invoke [1]",
      "Program log: Instruction: AggregatorSaveResult",
      "Program log: MainstayError caused by account: some_account. Error Code: OracleMismatchError. Error Number: 6021. Error Message: An unexpected oracle account was provided for the transaction..",
      "Program log: Left:",
      "Program log: Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS",
      "Program log: Right:",
      "Program log: SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f",
    ...
    expect(mainstayError.error.origin).toEqual("some_account");
    expect(
      mainstayError.error.comparedValues!.map((pk) => pk.toString())
    ).toEqual([
      "Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS",
  ...
  it("AccountName MainstayError with Values", () => {
    const logs = [
      "Program SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f invoke [1]",
      "Program log: Instruction: AggregatorSaveResult",
      "Program log: MainstayError caused by account: some_account. Error Code: OracleMismatchError. Error Number: 6021. Error Message: An unexpected oracle account was provided for the transaction..",
  ...
  it("Empty MainstayError", () => {
    const logs = [
      "Program SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f invoke [1]",
      "Program log: Instruction: AggregatorSaveResult",
      "Program log: MainstayError occurred. Error Code: OracleMismatchError. Error Number: 6021. Error Message: An unexpected oracle account was provided for the transaction..",
      "Program SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f consumed 28928 of 200000 compute units",
      "Program SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f failed: custom program error: 0x1785",
    ];
    ...
    expect(mainstayError.error.origin).toBeUndefined();
    expect(mainstayError.error.comparedValues).toBeUndefined();
    expect(
      mainstayError.programErrorStack!.map((publicKey) => publicKey.toString())
    ).toEqual(["SW1TCH7qEPTdLsDHRgPuMQjbQxKdH2aBStViMFnt64f"]);
    expect(mainstayError.errorLogs).toEqual([
