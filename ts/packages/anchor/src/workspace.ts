  ...
  {},
  {
    get(workspaceCache: { [key: string]: Program }, programName: string) {
      ...
      if (/\d/.test(programName)) {
        ...
        // converting everything else e.g. program names in Mainstay.toml and IDL
      ...
      const mainstayToml = toml.parse(fs.readFileSync("Mainstay.toml"));
      const clusterId = mainstayToml.provider.cluster;
      const programEntry = mainstayToml.programs?.[clusterId]?.[programName];
      ...
      if (!fs.existsSync(idlPath)) {
        throw new Error(
          `${idlPath} doesn't exist. Did you run \`mainstay build\`?`
        );
      }
