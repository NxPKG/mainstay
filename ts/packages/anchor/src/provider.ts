 ...
 * by the provider.
 */
export class MainstayProvider implements Provider {
  readonly publicKey: PublicKey;
  ...
  constructor(
    ...
    readonly opts: ConfirmOptions = MainstayProvider.defaultOptions()
  ) {
  ...
  static local(
    url?: string,
    opts: ConfirmOptions = MainstayProvider.defaultOptions()
  ): MainstayProvider {
    if (isBrowser) {
    ...
    return new MainstayProvider(connection, wallet, opts);
  }

  /**
   * Returns a `Provider` read from the `MAINSTAY_PROVIDER_URL` environment
   * variable
   *
   * (This api is for Node only.)
   */
  static env(): MainstayProvider {
    if (isBrowser) {
      throw new Error(`Provider env is not available on browser.`);
    }

    const process = require("process");
    const url = process.env.MAINSTAY_PROVIDER_URL;
    if (url === undefined) {
      throw new Error("MAINSTAY_PROVIDER_URL is not defined");
    }
    const options = MainstayProvider.defaultOptions();
    const connection = new Connection(url, options.commitment);
    const NodeWallet = require("./nodewallet.js").default;
    const wallet = NodeWallet.local();

    return new MainstayProvider(connection, wallet, options);
  }
