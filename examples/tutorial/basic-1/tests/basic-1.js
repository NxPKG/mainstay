...
describe("basic-1", () => {
  ...
  it("Creates and initializes an account in a single atomic transaction (simplified)", async () => {
    ...
    const program = mainstay.workspace.Basic1;
    ...
    await program.methods
      .initialize(new mainstay.BN(1234))
      .accounts({
    ...
    assert.ok(account.data.eq(new mainstay.BN(1234)));
  ...
  it("Updates a previously created account", async () => {
    ...
    assert.ok(account.data.eq(new mainstay.BN(4321)));
