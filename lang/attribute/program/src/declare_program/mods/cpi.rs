    ...
    let ixs = idl.instructions.iter().map(|ix| {
        ...
        let (ret_type, ret_value) = match ix.returns.as_ref() {
            Some(ty) => {
                ...
                (
                    quote! { mainstay_lang::Result<Return::<#ty>> },
                    quote! { Ok(Return::<#ty> { phantom: std::marker::PhantomData }) },
                )
            },
            None => (
                quote! { mainstay_lang::Result<()> },
                quote! { Ok(()) },
            )
        };
        ...
        quote! {
            pub fn #method_name<'a, 'b, 'c, 'info>(
                ctx: mainstay_lang::context::CpiContext<'a, 'b, 'c, 'info, accounts::#accounts_ident<'info>>,
                #(#args),*
            ) -> #ret_type {
                let ix = {
                    let mut data = Vec::with_capacity(256);
                    data.extend_from_slice(&#discriminator);
                    MainstaySerialize::serialize(&internal::args::#arg_value, &mut data)
