use mainstay_lang_idl::types::{Idl, IdlInstructionAccountItem};
use mainstay_syn::{
...
fn gen_internal_args_mod(idl: &Idl) -> proc_macro2::TokenStream {
    let ixs = idl.instructions.iter().map(|ix| {
        ...
        let impl_discriminator = if ix.discriminator.len() == 8 {
            ...
            quote! {
                impl mainstay_lang::Discriminator for #ix_struct_name {
        ...
        let impl_ix_data = quote! {
            impl mainstay_lang::InstructionData for #ix_struct_name {}
        };

        let program_id = get_canonical_program_id();
        let impl_owner = quote! {
            impl mainstay_lang::Owner for #ix_struct_name {
                fn owner() -> Pubkey {
                    #program_id
                }
            }
    ...
    quote! {
        /// An Mainstay generated module containing the program's set of instructions, where each
        ...
        /// Mainstay instruction data, for example, when specifying instructions instructions on a
        /// client.
        pub mod args {
            use super::*;
