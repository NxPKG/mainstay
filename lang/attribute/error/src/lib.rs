...
use mainstay_syn::codegen;
use mainstay_syn::parser::error::{self as error_parser, ErrorInput};
use mainstay_syn::ErrorArgs;
use syn::{parse_macro_input, Expr};

/// Generates `Error` and `type Result<T> = Result<T, Error>` types to be
/// used as return types from Mainstay instruction handlers. Importantly, the
...
/// Generates an [`Error::MainstayError`](../../mainstay_lang/error/enum.Error.html) that includes file and line information.
///
/// # Example
/// ```rust,ignore
/// #[program]
...
fn create_error(error_code: Expr, source: bool, account_name: Option<Expr>) -> TokenStream {
    let error_origin = match (source, account_name) {
        ...
        (false, Some(account_name)) => quote! {
            Some(mainstay_lang::error::ErrorOrigin::AccountName(#account_name.to_string()))
        },
        (true, _) => quote! {
            Some(mainstay_lang::error::ErrorOrigin::Source(mainstay_lang::error::Source {
                filename: file!(),
                line: line!()
            }))
        },
    ...
    TokenStream::from(quote! {
        mainstay_lang::error::Error::from(
            mainstay_lang::error::MainstayError {
