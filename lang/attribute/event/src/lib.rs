...
use mainstay_syn::parser::accounts::event_cpi::{add_event_cpi_accounts, EventAuthority};
use quote::quote;
...
pub fn event(
    ...
    let discriminator: proc_macro2::TokenStream = {
        ...
        let discriminator = mainstay_syn::hash::hash(&discriminator_preimage);
        format!("{:?}", &discriminator.0[..8]).parse().unwrap()
    };

    let ret = quote! {
        #[derive(mainstay_lang::__private::EventIndex, MainstaySerialize, MainstayDeserialize)]
        #event_strct

        impl mainstay_lang::Event for #event_name {
            fn data(&self) -> Vec<u8> {
        ...
        impl mainstay_lang::Discriminator for #event_name {
    ...
    {
        let idl_build = mainstay_syn::idl::gen_idl_print_fn_event(&event_strct);
...
pub fn emit(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    proc_macro::TokenStream::from(quote! {
        {
            mainstay_lang::solana_program::log::sol_log_data(&[&mainstay_lang::Event::data(&#data)]);
        }
    })
}
    ...
    proc_macro::TokenStream::from(quote! {
        {
            ...
            let disc = mainstay_lang::event::EVENT_IX_TAG_LE;
            let inner_data = mainstay_lang::Event::data(&#event_struct);
            let ix_data: Vec<u8> = disc.into_iter().chain(inner_data.into_iter()).collect();

            let ix = mainstay_lang::solana_program::instruction::Instruction::new_with_bytes(
                crate::ID,
                &ix_data,
                vec![
                    mainstay_lang::solana_program::instruction::AccountMeta::new_readonly(
                        *authority_info.key,
                        true,
                    ),
                ],
            ...
            .map_err(mainstay_lang::error::Error::from)?;
        }
    })
}
