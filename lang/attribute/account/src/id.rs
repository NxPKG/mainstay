//! Copied from solana/sdk/macro so that Mainstay programs don't need to specify
//! `solana_program` as an additional crate dependency, but instead can access
//! it via `mainstay_lang::declare_id`.
//!
...
impl Parse for Id {
    fn parse(input: ParseStream) -> Result<Self> {
        parse_id(
            input,
            quote! { mainstay_lang::solana_program::pubkey::Pubkey },
        )
        .map(Self)
    }
}
...
impl Parse for Pubkeys {
    fn parse(input: ParseStream) -> Result<Self> {
        let pubkey_type = quote! {
            mainstay_lang::solana_program::pubkey::Pubkey
        };
