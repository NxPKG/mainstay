...
use mainstay_syn::{codegen::program::common::gen_discriminator, Overrides};
...
/// - [`MainstaySerialize`](./trait.MainstaySerialize.html)
/// - [`MainstayDeserialize`](./trait.MainstayDeserialize.html)
...
pub fn account(
    ...
    let owner_impl = {
        if namespace.is_empty() {
            quote! {
                ...
                impl #impl_gen mainstay_lang::Owner for #account_name #type_gen #where_clause {
                    fn owner() -> Pubkey {
                        crate::ID
                    }
                }
    ...
    let unsafe_bytemuck_impl = {
        if unsafe_bytemuck {
            quote! {
                #[automatically_derived]
                unsafe impl #impl_gen mainstay_lang::__private::bytemuck::Pod for #account_name #type_gen #where_clause {}
                #[automatically_derived]
                unsafe impl #impl_gen mainstay_lang::__private::bytemuck::Zeroable for #account_name #type_gen #where_clause {}
            }
        } else {
            quote! {}
        }
    ...
    proc_macro::TokenStream::from({
        if is_zero_copy {
            quote! {
                ...
                #[automatically_derived]
                impl #impl_gen mainstay_lang::ZeroCopy for #account_name #type_gen #where_clause {}

                #[automatically_derived]
                impl #impl_gen mainstay_lang::Discriminator for #account_name #type_gen #where_clause {
                ...
                #[automatically_derived]
                impl #impl_gen mainstay_lang::AccountDeserialize for #account_name #type_gen #where_clause {
                    fn try_deserialize(buf: &mut &[u8]) -> mainstay_lang::Result<Self> {
                        ...
                        if #disc != given_disc {
                            return Err(mainstay_lang::error!(mainstay_lang::error::ErrorCode::AccountDiscriminatorMismatch).with_account_name(#account_name_str));
                        }
            ...
            quote! {
                ...
                #[automatically_derived]
                impl #impl_gen mainstay_lang::AccountSerialize for #account_name #type_gen #where_clause {
    ...
    let methods: Vec<proc_macro2::TokenStream> = fields
        ...
        .filter_map(|field: &syn::Field| {
            field
                .attrs
                .iter()
                .find(|attr| mainstay_syn::parser::tts_to_string(&attr.path) == "accessor")
                .map(|attr| {
                    ...
                    quote! {
                        pub fn #get_field(&self) -> #accessor_ty {
                            mainstay_lang::__private::ZeroCopyAccessor::get(&self.#field_name)
                        }
                        pub fn #set_field(&mut self, input: &#accessor_ty) {
                            self.#field_name = mainstay_lang::__private::ZeroCopyAccessor::set(input);
                        }
                    }
                })
        })
...
pub fn zero_copy(
    ...
    let attr = account_strct
        .attrs
        .iter()
        .find(|attr| mainstay_syn::parser::tts_to_string(&attr.path) == "repr");
    ...
    // exported mainstay bytemuck `__private::bytemuck`, so that there won't be any
    // possible disparity between the mainstay version and the local crate's version.
    ...
    let ret = quote! {
        #[derive(mainstay_lang::__private::ZeroCopyAccessor, Copy, Clone)]
        #repr
        #pod
        #zeroable
        #account_strct
    ...
    #[cfg(feature = "idl-build")]
    {
        ...
        .unwrap();
        let idl_build_impl = mainstay_syn::idl::impl_idl_build_struct(&zc_struct);
...
/// Defines the program's ID. This should be used at the root of all Mainstay
/// based programs.
#[proc_macro]
pub fn declare_id(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    #[cfg(feature = "idl-build")]
    {
        let idl_print = mainstay_syn::idl::gen_idl_print_fn_address(address);
        return proc_macro::TokenStream::from(quote! {
            #ret
            #idl_print
        });
