...
pub fn gen_idl_print_fn_program(program: &Program) -> TokenStream {
    ...
    let result = program
        ...
        .map(|ix| {
            ...
            let ctx_ident = &ix.mainstay_ident;
            let cfgs = &ix.cfgs;
            ...
            Ok((
                quote! {
                    ...
                    #idl::IdlInstruction {
                        ...
                        accounts: #ctx_ident::__mainstay_private_gen_idl_accounts(
                            &mut accounts,
                            &mut types,
                        ),
                        args: vec![#(#args),*],
    ...
    quote! {
        #[test]
        pub fn __mainstay_private_print_idl_program() {
            #fn_body
        }
    }
}
...
fn check_safety_comments() -> Result<()> {
    let skip_lint = option_env!("MAINSTAY_IDL_BUILD_SKIP_LINT")
        .map(|val| val == "TRUE")
        .unwrap_or_default();
    if skip_lint {
        return Ok(());
    ...
    if program_path.is_err() {
        ...
        //
        // - Mainstay CLI version is incompatible with the current version
        // - The error is coming from Rust Analyzer when the user has `idl-build` feature enabled,
        // likely due to enabling all features (https://github.com/nxpkg/mainstay/issues/3042)
        //
        ...
        // settings.
        //
        // Given this feature is not a critical one, and it works by default with `mainstay build`,
