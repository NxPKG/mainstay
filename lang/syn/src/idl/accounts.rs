...
pub fn gen_idl_build_impl_accounts_struct(accounts: &AccountsStruct) -> TokenStream {
    let resolution = option_env!("MAINSTAY_IDL_BUILD_RESOLUTION")
        .map(|val| val == "TRUE")
        .unwrap_or_default();
        ...
        .map(|acc| match acc {
            AccountField::Field(acc) => {
                ...
                let acc_type_path = match &acc.ty {
                    ...
                    // we wrap the type in order to implement `IdlBuild` in `mainstay-lang`.
                        if !ty
                            .account_type_path
                            .path
                            .to_token_stream()
                ...
                (
                    quote! {
                        #idl::IdlInstructionAccountItem::Composite(#idl::IdlInstructionAccounts {
                            name: #name.into(),
                            accounts: <#ty>::__mainstay_private_gen_idl_accounts(accounts, types),
                        })
                    },
                    None,
                )
    ...
    quote! {
        impl #impl_generics #ident #ty_generics #where_clause {
            pub fn __mainstay_private_gen_idl_accounts(
    ...
    let pda = acc
        ...
        .and_then(|(wallet, mint, token_program)| {
            ...
            let token_program = token_program
                .as_ref()
                .and_then(parse_ata)
                .or_else(|| parse_expr(quote!(mainstay_spl::token::ID)));
            ...
            let program = parse_expr(quote!(mainstay_spl::associated_token::ID))
                .map(|program| quote! { Some(#program) })
                .unwrap();
