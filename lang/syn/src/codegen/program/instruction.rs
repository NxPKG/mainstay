...
pub fn generate(program: &Program) -> proc_macro2::TokenStream {
    let variants: Vec<proc_macro2::TokenStream> = program
        ...
        .map(|ix| {
            ...
            let impls = {
                ...
                quote! {
                    #(#ix_cfgs)*
                    impl mainstay_lang::Discriminator for #ix_name_camel {
                        const DISCRIMINATOR: &'static [u8] = #discriminator;
                    }
                    #(#ix_cfgs)*
                    impl mainstay_lang::InstructionData for #ix_name_camel {}
                    #(#ix_cfgs)*
                    impl mainstay_lang::Owner for #ix_name_camel {
                        fn owner() -> Pubkey {
                            ID
                        }
                    }
    ...
    quote! {
        /// An Mainstay generated module containing the program's set of
        /// instructions, where each method handler in the `#[program]` mod is
        /// associated with a struct defining the input arguments to the
        /// method. These should be used directly, when one wants to serialize
        /// Mainstay instruction data, for example, when speciying
