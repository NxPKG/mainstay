...
pub fn generate(program: &Program) -> proc_macro2::TokenStream {
    ...
    let fallback_fn = gen_fallback(program).unwrap_or(quote! {
        Err(mainstay_lang::error::ErrorCode::InstructionFallbackNotFound.into())
    });
    ...
    quote! {
        ...
        ///
        /// Each method in an mainstay program is uniquely defined by a namespace
        /// and a rust identifier (i.e., the name given to the method). These
        /// two pieces can be combined to create a method identifier,
        /// specifically, Mainstay uses
        ///
        ...
        /// regular instruction.
        ///
        /// With this 8 byte identifier, Mainstay performs method dispatch,
        ...
        fn dispatch<'info>(
            ...
            use mainstay_lang::Discriminator;
            match sighash {
                #(#global_dispatch_arms)*
                mainstay_lang::idl::IDL_IX_TAG_LE => {
                    // If the method identifier is the IDL tag, then execute an IDL
                    // instruction, injected into all Mainstay programs unless they have
                    // no-idl enabled
                    ...
                    #[cfg(feature = "no-idl")]
                    {
                        Err(mainstay_lang::error::ErrorCode::IdlInstructionStub.into())
                    }
                }
                mainstay_lang::event::EVENT_IX_TAG_LE => {
                    #event_cpi_handler
                }
                _ => {
                    #fallback_fn
...
pub fn generate_event_cpi_handler() -> proc_macro2::TokenStream {
    ...
    quote! {
        ...
        Err(mainstay_lang::error::ErrorCode::EventInstructionStub.into())
    }
}
