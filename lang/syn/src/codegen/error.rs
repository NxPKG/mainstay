...
pub fn generate(error: Error) -> proc_macro2::TokenStream {
    ...
    let offset = match &error.args {
        None => quote! { mainstay_lang::error::ERROR_CODE_OFFSET},
        Some(args) => {
    ...
    let ret = quote! {
        ...
        impl From<#enum_name> for mainstay_lang::error::Error {
            fn from(error_code: #enum_name) -> mainstay_lang::error::Error {
