...
pub fn generate(accs: &AccountsStruct) -> proc_macro2::TokenStream {
    ...
    let on_save: Vec<proc_macro2::TokenStream> = accs
        ...
        .map(|af: &AccountField| match af {
            AccountField::CompositeField(s) => {
                ...
                quote! {
                    mainstay_lang::AccountsExit::exit(&self.#name, program_id)
                ...
                if f.constraints.is_close() {
                    ...
                    quote! {
                        {
                            ...
                            mainstay_lang::AccountsClose::close(
                                &self.#ident,
                    ...
                    match f.constraints.is_mutable() {
                        ...
                        true => match &f.ty {
                            ...
                            _ => quote! {
                                mainstay_lang::AccountsExit::exit(&self.#ident, program_id)
                                    .map_err(|e| e.with_account_name(#name_str))?;
                            },
                        },
                    }
    ...
    quote! {
        ...
        impl<#combined_generics> mainstay_lang::AccountsExit<#trait_generics> for #name<#struct_generics> #where_clause{
            fn exit(&self, program_id: &mainstay_lang::solana_program::pubkey::Pubkey) -> mainstay_lang::Result<()> {
                #(#on_save)*
                Ok(())
            }
        }
