...
/// - [Using Account with non-mainstay types](#using-account-with-non-mainstay-types)
...
/// ```
///
/// # Using Account with non-mainstay programs
///
/// Account can also be used with non-mainstay programs. The data types from
...
/// to write.
///
/// The mint wrapper type that Mainstay provides out of the box for the token program ([source](https://github.com/nxpkg/mainstay/blob/master/spl/src/token.rs))
/// ```ignore
/// #[derive(Clone)]
/// pub struct Mint(spl_token::state::Mint);
///
/// // This is necessary so we can use "mainstay_spl::token::Mint::LEN"
/// // because rust does not resolve "mainstay_spl::token::Mint::LEN" to
...
/// // because non-mainstay accounts don't have a discriminator to check
/// impl mainstay_lang::AccountDeserialize for Mint {
...
/// impl mainstay_lang::AccountSerialize for Mint {}
///
/// impl mainstay_lang::Owner for Mint {
///     fn owner() -> Pubkey {
...
///
/// Mainstay provides wrapper types to access data stored in programs owned by the BPFUpgradeableLoader
...
/// ```
/// to let mainstay do the matching for you and return the ProgramData variant of BpfUpgradeableLoaderState.
///
/// # Example
/// ```ignore
/// use mainstay_lang::prelude::*;
...
/// ### SPL Types
///
/// Mainstay provides wrapper types to access accounts owned by the token program. Use
/// ```ignore
