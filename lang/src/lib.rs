...
//! Mainstay ⚓ is a framework for Solana's Sealevel runtime providing several
...
//!
//! For detailed tutorials and examples on how to use Mainstay, see the guided
//! [tutorials](https://mainstay-lang.com) or examples in the GitHub
//! [repository](https://github.com/nxpkg/mainstay).
//!
//! Presented here are the Rust primitives for building on Solana.

extern crate self as mainstay_lang;
...
pub use borsh::de::BorshDeserialize as MainstayDeserialize;
pub use borsh::ser::BorshSerialize as MainstaySerialize;
pub use solana_program;

#[cfg(feature = "event-cpi")]
pub use mainstay_attribute_event::{emit_cpi, event_cpi};

#[cfg(feature = "idl-build")]
pub use idl::IdlBuild;

#[cfg(feature = "interface-instructions")]
pub use mainstay_attribute_program::interface;
...
pub trait Event: MainstaySerialize + MainstayDeserialize + Discriminator {
    fn data(&self) -> Vec<u8>;
}
...
/// All programs should include it via `mainstay_lang::prelude::*;`.
pub mod prelude {
    pub use super::{
...
macro_rules! require {
    ($invariant:expr, $error:tt $(,)?) => {
        if !($invariant) {
            return Err(mainstay_lang::error!($crate::ErrorCode::$error));
        }
    };
    ($invariant:expr, $error:expr $(,)?) => {
        if !($invariant) {
            return Err(mainstay_lang::error!($error));
        }
    };
}
...
macro_rules! require_eq {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 != $value2 {
            return Err(error!(mainstay_lang::error::ErrorCode::RequireEqViolated)
...
macro_rules! require_neq {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 == $value2 {
            return Err(error!(mainstay_lang::error::ErrorCode::RequireNeqViolated)
                .with_values(($value1, $value2)));
        }
    };
}
...
macro_rules! require_keys_neq {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 == $value2 {
            return Err(
                error!(mainstay_lang::error::ErrorCode::RequireKeysNeqViolated)
                    .with_pubkeys(($value1, $value2)),
            );
        }
    };
...
macro_rules! require_gt {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 <= $value2 {
            return Err(error!(mainstay_lang::error::ErrorCode::RequireGtViolated)
                .with_values(($value1, $value2)));
        }
    };
}
...
macro_rules! require_gte {
    ...
    ($value1: expr, $value2: expr $(,)?) => {
        if $value1 < $value2 {
            return Err(error!(mainstay_lang::error::ErrorCode::RequireGteViolated)
                .with_values(($value1, $value2)));
        }
    };
}
...
macro_rules! err {
    ($error:tt $(,)?) => {
        Err(mainstay_lang::error!($crate::ErrorCode::$error))
    };
    ($error:expr $(,)?) => {
        Err(mainstay_lang::error!($error))
    };
}
