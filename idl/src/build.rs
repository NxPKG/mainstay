...
///
/// This trait is automatically implemented for Mainstay all types that use the `MainstaySerialize`
/// proc macro. Note that manually implementing the `MainstaySerialize` trait does **NOT** have the
/// same effect.
///
...
fn build(
    ...
    let output = Command::new("cargo")
        ...
        .env(
            "MAINSTAY_IDL_BUILD_NO_DOCS",
            if no_docs { "TRUE" } else { "FALSE" },
        )
        .env(
            "MAINSTAY_IDL_BUILD_RESOLUTION",
            if resolution { "TRUE" } else { "FALSE" },
        )
        .env(
            "MAINSTAY_IDL_BUILD_SKIP_LINT",
            if skip_lint { "TRUE" } else { "FALSE" },
        )
        .env("MAINSTAY_IDL_BUILD_PROGRAM_PATH", program_path)
    ...
    if env::var("MAINSTAY_LOG").is_ok() {
        eprintln!("{}", stdout);
    }

    if !output.status.success() {
        return Err(anyhow!(
            "Building IDL failed. Run `MAINSTAY_LOG=true mainstay idl build` to see the logs."
        ));
    }
