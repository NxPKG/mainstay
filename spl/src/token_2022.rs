...
pub fn initialize_account<'info>(
    ...
    mainstay_lang::solana_program::program::invoke(
        &ix,
        &[
...
pub fn initialize_account3<'info>(
    ...
    mainstay_lang::solana_program::program::invoke(&ix, &[ctx.accounts.account, ctx.accounts.mint])
        .map_err(Into::into)
}
...
pub fn initialize_mint<'info>(
    ...
    mainstay_lang::solana_program::program::invoke(&ix, &[ctx.accounts.mint, ctx.accounts.rent])
        .map_err(Into::into)
}
...
pub fn initialize_mint2<'info>(
    ...
    mainstay_lang::solana_program::program::invoke(&ix, &[ctx.accounts.mint]).map_err(Into::into)
}
...
pub fn sync_native<'info>(ctx: CpiContext<'_, '_, '_, 'info, SyncNative<'info>>) -> Result<()> {
    ...
    mainstay_lang::solana_program::program::invoke(&ix, &[ctx.accounts.account]).map_err(Into::into)
}
...
pub fn get_account_data_size<'info>(
    ...
    mainstay_lang::solana_program::program::invoke(&ix, &[ctx.accounts.mint])?;
    mainstay_lang::solana_program::program::get_return_data()
        .ok_or(mainstay_lang::solana_program::program_error::ProgramError::InvalidInstructionData)
        .and_then(|(key, data)| {
            if key != *ctx.program.key {
                Err(mainstay_lang::solana_program::program_error::ProgramError::IncorrectProgramId)
            } else {
                data.try_into().map(u64::from_le_bytes).map_err(|_| {
                    mainstay_lang::solana_program::program_error::ProgramError::InvalidInstructionData
                })
            }
        })
        .map_err(Into::into)
...
pub fn amount_to_ui_amount<'info>(
    ...
    mainstay_lang::solana_program::program::invoke(&ix, &[ctx.accounts.account])?;
    mainstay_lang::solana_program::program::get_return_data()
        .ok_or(mainstay_lang::solana_program::program_error::ProgramError::InvalidInstructionData)
        .and_then(|(key, data)| {
            if key != *ctx.program.key {
...
impl mainstay_lang::Id for Token2022 {
    fn id() -> Pubkey {
        ID
    }
}
