...
# Replace only matching "version": "x.xx.x" and "@nx-pkg/mainstay": "x.xx.x"
git grep -l $(cat VERSION) -- "**/package.json" | \
    xargs sed "${sedi[@]}" \
    -e "s/@nx-pkg\/mainstay\": \"$(cat VERSION)\"/@nx-pkg\/mainstay\": \"$version\"/g" \
...
pushd tests/bench && mainstay run bump-version -- --mainstay-version $version && popd
